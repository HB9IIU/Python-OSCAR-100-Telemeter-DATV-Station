...
else
{
    ...
    var uplink_frequency_daniel=2407.250;
    var downlink_frequency_daniel=10496.750;
    var symbol_rate_daniel = 333;
    document.getElementById("label_uplink_frequency").innerHTML = uplink_frequency_daniel.toFixed(3).toString().replace(/\B(?=(\d{3})+(?!\d))/g, "'")+ " Mhz" ;
    document.getElementById("label_downlink_frequency").innerHTML = downlink_frequency_daniel.toFixed(3).toString().replace(/\B(?=(\d{3})+(?!\d))/g, "'")+ " Mhz" ;
    document.getElementById("label_symbol_rate").innerHTML = symbol_rate_daniel.toFixed(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, "'")+ " kS"
    ...
    function signal_steering(f,sr) { //F5UII 2608
        ...
        console.log("I am here (x) in wf.js",f,sr);
        let rx_downlink_frequ=f+10000;
        console.log(rx_downlink_frequ);
        let rx_corresponding_uplink_frequency=rx_downlink_frequ-8089.5;
        console.log(rx_corresponding_uplink_frequency);
        downlink_frequency_daniel=rx_downlink_frequ;
        uplink_frequency_daniel=rx_corresponding_uplink_frequency;
        symbol_rate_daniel=sr;
        update_frequencies_daniel();
    ...
    function updateFFT(data)
    {
       ...
       // ctx.fillText("(1.5MS/s QPSK, 4/5)",((491.5)-_start_freq)*(canvasWidth/9),canvasHeight-12);
        ...
        if(data != null)
        {
            ...
            grad.addColorStop(0.2, "#ee0505");
            grad.addColorStop(1, "#388f0c");
    ...
    function copy_upfreq(mouse_x, mouse_y)
    {
        if(mouse_y > (canvasHeight * 7/8))
        {
           ...
           // $(t+'#f-central').val((parseFloat($("#upf").val()- $(t+'[name="trvlo"]').val())));
            ...
            console.log("I am here (1) in wf.js ", uplink_frequency_daniel);
            ...
            for(var i = 0; i < freq_info.length; i ++)
            {
                    ...
                    &&(mouse_y > yd-5) && (mouse_y < yd+5) )
                {
                    ...
                    //document.getElementById("upf").value = (1910.50 + freq_info[i].center_frequency ) ;
                     uplink_frequency_daniel =(1910.50 + freq_info[i].center_frequency );
                   downlink_frequency_daniel = uplink_frequency_daniel+8089.5;
                    //console.log(uplink_frequency_daniel)
                    // daniel
...
function update_frequencies_daniel() {
    console.log("here hihi (function update_frequencies_daniel()) in wf.js")
    document.getElementById("label_uplink_frequency").innerHTML = uplink_frequency_daniel.toFixed(3).toString().replace(/\B(?=(\d{3})+(?!\d))/g, "'")+ " Mhz" ;
    document.getElementById("label_downlink_frequency").innerHTML = downlink_frequency_daniel.toFixed(3).toString().replace(/\B(?=(\d{3})+(?!\d))/g, "'")+ " Mhz" ;
    document.getElementById("label_symbol_rate").innerHTML = symbol_rate_daniel.toFixed(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, "'")+ " kS" ;
    switch(symbol_rate_daniel) {
        case 66:
            $('#SR66').prop('checked', true);
            break;
        case 125:
            $('#SR125').prop('checked', true);
            break;
        case 250:
            $('#SR250').prop('checked', true);
            break;
        case 333:
            $('#SR333').prop('checked', true);
            break;
        case 500:
            $('#SR500').prop('checked', true);
            break;
        case 1000:
            $('#SR1000').prop('checked', true);
            break;
        case 1500:
            $('#SR1500').prop('checked', true);
    }
    $.ajax({
        url: '/askpython',
        type: "POST",
        data: JSON.stringify({Instruction: "Set_Minitiouner", Argument_1: uplink_frequency_daniel, Argument_2: symbol_rate_daniel}),
